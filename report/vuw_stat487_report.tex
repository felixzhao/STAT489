...
In this work we split simulation data to training and validation set.

First, EM algorithm has been used to Calculate each parameters value from the training data ($Y$ only).

Then, use the algorithm in Algorithm~\ref{fig:algo} to Calculate the probability of each of categories, cluter and column combination.
All the probabilities in each cluster are standardized.

At last, for each new observations $Y'$ in validation set are calculate Z score of each category by this probabilities.
