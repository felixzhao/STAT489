...
\begin{enumerate}
    ...
    \item \textbf{Detailed Parameter Analysis}: We conduct a comprehensive examination of the components of the probability function, including the cluster effect, column effect, and covariate effect. This detailed analysis allows us to better understand how these parameters contribute to the clustering process, providing deeper insights into the interaction between different factors in the model.
...
\begin{equation}
  \log\left(\frac{P(Y = k)}{P(Y = 1)} \mid k \in C, g \in G, i \in n, j \in m\right) = \mu_k + \phi_k \times \left(\alpha_g + \beta_j + X_i^T \cdot \theta \right),
\end{equation}
where:
...
\begin{itemize}
    ...
    \item \(X_i^T \cdot \theta\) represents the effect associated with row \(i\), where \(\theta\) is a vector of coefficients.
\end{itemize}
...
\begin{figure}[ht]
  ...
  \begin{subfigure}{0.49\textwidth}
      \centering
      \includegraphics[width=\textwidth]{images/experiments/exp_diff_clusters_2.png} % Replace with your second image path
...
\subsubsection*{Normal Distribution simulation for 5 clusters with 100 Ys}
...
\subsection{The effect to results from number of columns}

TODO

\subsection{Analysis of 8-Cluster Prediction Performance}
